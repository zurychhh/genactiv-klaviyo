...
type BulkUpdateSeoInput = z.infer<typeof BulkUpdateSeoInputSchema>;
...
const bulkUpdateSeo = {
  ...
  execute: async (input: BulkUpdateSeoInput) => {
    ...
    for (const item of items) {
      ...
      try {
        ...
        if (item.metaTitle !== undefined) seo.title = item.metaTitle;
        if (item.metaDescription !== undefined) seo.description = item.metaDescription;
        ...
        if (item.type === "product") {
          const data = (await shopifyClient.request(PRODUCT_UPDATE_MUTATION, {
            ...
            productUpdate: {
              product: { id: string; title: string; handle: string; seo: { title: string | null; description: string | null } };
          ...
          const data = (await shopifyClient.request(COLLECTION_UPDATE_MUTATION, {
            ...
            collectionUpdate: {
              collection: { id: string; title: string; handle: string; seo: { title: string | null; description: string | null } };
              userErrors: Array<{ field: string; message: string }>;
            };
          };
