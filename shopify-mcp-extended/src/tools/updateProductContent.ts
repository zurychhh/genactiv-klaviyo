...
type UpdateProductContentInput = z.infer<typeof UpdateProductContentInputSchema>;
...
const updateProductContent = {
  ...
  execute: async (input: UpdateProductContentInput) => {
    try {
      ...
      const fetchData = (await shopifyClient.request(fetchQuery, { id: productGid })) as {
        product: {
          id: string;
          title: string;
          handle: string;
      ...
      const data = (await shopifyClient.request(updateQuery, { input: productInput })) as {
        productUpdate: {
          product: {
            id: string;
            title: string;
      ...
      throw new Error(
        `Failed to update product content: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  },
};
