...
const updateProductImages = {
    ...
    execute: async (input) => {
      ...
      `;
            const productData = (await shopifyClient.request(getQuery, { id: productGid }));
            if (!productData.product) {
                throw new Error(`Product not found: ${productId}`);
            }
            const existingImages = productData.product.images.edges.map((e) => e.node);
            ...
            for (const img of images) {
                ...
                const existingImage = existingImages.find((ei) => {
          ...
          `;
            ...
            throw new Error(`Failed to update product images: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
};
