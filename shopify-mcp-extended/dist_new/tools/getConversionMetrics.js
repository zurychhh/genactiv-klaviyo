...
const getConversionMetrics = {
    ...
    execute: async (input) => {
        try {
            ...
            const byFinancialStatus = {};
            const byFulfillmentStatus = {};
            const byPaymentGateway = {};
            ...
            for (const edge of edges) {
                ...
                // Count items
                const itemCount = order.lineItems.edges.reduce((sum, item) => sum + item.node.quantity, 0);
            ...
            // Format output
            const formatStatusData = (data) => Object.entries(data)
            ...
            throw new Error(`Failed to get conversion metrics: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
};
