...
/* A/B meta strip — pokazuje testowany Temat + Podgląd nad mailem */
function ABMeta({ variant, subject, preheader }) {
  return (
    <div style={{ background: '#1C1B1B', color: '#fff', padding: '12px 20px', fontFamily: 'var(--ga-font-body)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
        <span style={{ background: 'var(--ga-red)', color: '#fff', fontSize: 10, fontWeight: 800, letterSpacing: '.1em', padding: '3px 8px', borderRadius: 999 }}>WARIANT {variant}</span>
        <span style={{ fontSize: 10, letterSpacing: '.08em', textTransform: 'uppercase', color: 'rgba(255,255,255,.5)' }}>Test A/B · temat + preheader</span>
      </div>
      <div style={{ fontSize: 13, fontWeight: 700, lineHeight: 1.35 }}>{subject}</div>
      <div style={{ fontSize: 11, color: 'rgba(255,255,255,.6)', marginTop: 3 }}>{preheader}</div>
    </div>
  );
}
...
Object.assign(window, { EIcon, Preheader, ABMeta, EmailFooter });
